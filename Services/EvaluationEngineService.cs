...
namespace Dignus.Candidate.Back.Services
{
    ...
    public class EvaluationEngineService : IEvaluationEngineService
    {
        ...
        public int CalculateCompletionPercentage(MathTest? mathTest, PortugueseTest? portugueseTest, PsychologyTest? psychologyTest, List<VideoInterview> videoInterviews)
        {
            int completed = 0;
            int total = 4; // Math, Portuguese, Psychology, Video

            if (mathTest != null && mathTest.Status == TestStatus.Submitted) completed++;
            if (portugueseTest != null && portugueseTest.Status == TestStatus.Submitted) completed++;
            if (psychologyTest != null && psychologyTest.Status == TestStatus.Submitted) completed++;
            if (videoInterviews.Any()) completed++;
