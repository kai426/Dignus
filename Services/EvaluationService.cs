...
namespace Dignus.Candidate.Back.Services
{
    ...
    public class EvaluationService : IEvaluationService
    {
        ...
        public async Task<decimal> CalculateOverallScoreAsync(Guid candidateId)
        {
            try
            {
                var tests = await _testService.GetTestsByCandidateIdAsync(candidateId);
                var completedTests = tests.Where(t => t.Status == TestStatus.Submitted && t.Score.HasValue).ToList();
        ...
        public async Task<bool> IsReadyForEvaluationAsync(Guid candidateId)
        {
            try
            {
                var testStatus = await _testService.GetTestStatusAsync(candidateId);
                
                // Check if all required tests are completed
                return testStatus["Math"] && testStatus["Portuguese"] && testStatus["Psychology"];
            }
            catch (Exception ex)
            {
